# Effect

Any DOT from [Skills](./Skill.md)
Any Buff from Skills too

Ideas: [idea page](https://tvtropes.org/pmwiki/pmwiki.php/Main/StatusEffects)

## Extends
 - #TODO: does this need to be catagorized?
 - statEffect - changes stat points (-50% speed or something)
 - dotEffect - ticks every x seconds to give you a health change. (Regeneration, Poison)

 - Sleep? effects that control actions?

## Properties
 - UHHHHHHHHHH TODO



### References
 - [Skill](./Skill.md)
